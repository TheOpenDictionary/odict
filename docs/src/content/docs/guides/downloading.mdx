...
ODict can load dictionaries from the remote registry as well as from local files. Remote names use the form `dictionary/language`, for example
`wiktionary/eng`.
...
By default, downloaded dictionaries are cached in ODict's config directory under `dictionaries/<dictionary>/`. The command stores the downloaded file
as `<language>.odict`.
...
:::note
`OpenDictionary.load()` tries to treat `dictionary/language` values as remote names. If the string is not a remote name, ODict falls back to aliases
and local paths when those features are available.
:::
