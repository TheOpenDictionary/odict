...
:::tip
When `follow` is enabled, ODict walks the `see` chain until it finds an entry with etymologies. It also detects circular references and returns an
...
If a term isn't found, ODict can split it into substrings and look up each part. The `split` parameter sets the minimum character length for each
fragment.
